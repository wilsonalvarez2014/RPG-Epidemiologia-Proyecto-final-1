/*:
 * @plugindesc [v2.0] QUIZ de or tomar agua del rio porvenir
 * @author Luis
 * 
 * 
 ...
 *      $gameSystem.juegoCompletadooraguadelrio()
 */
...
(function() {
    ...
    Object.defineProperty(window, 'oraguadelrio', {
        get: function() {
            return $gameVariables.value(177);
        },
        set: function(valor) {
            $gameVariables.setValue(177, valor);
        }
    });
    ...
    Game_System.prototype.juegoCompletadooraguadelrio = function() {
        return this._juegoCompleto22oraguadelrio === true;
    };

    Game_System.prototype.establecerjuegoCompletadooraguadelrio = function(estado14) {
        this._juegoCompleto22oraguadelrio = estado14;
    };

    Game_System.prototype.juegoBloqueadooraguadelrio = function() {
        return this._juegoBloqueado22oraguadelrio === true;
    };

    Game_System.prototype.establecerBloqueooraguadelrio = function(estado24) {
        this._juegoBloqueado22oraguadelrio = estado24;
    };
    ...
    Game_Interpreter.prototype.pluginCommand = function(comando, args) {
        ...
        if (comando === "Iniciaroraguadelrio") {
            if ($gameSystem.lrmzJuegoBloqueadooraguadelrio()) {
                SceneManager.push(Scene_LRMZ_BloqueoFinaloraguadelrio);
                return;
            }
            
            if ($gameSystem.juegoCompletadooraguadelrio()) {
                alert("Ya completaste este desafío con la máxima puntuación.");
                return;
            }
            // variable contador 2
            if ($gameVariables.value(178) === 0) {
                $gameVariables.setValue(178, 10) 
            }
            //establecer variable de juego
            if ($gameVariables.value(177) === 0) {
                $gameVariables.setValue(177, 10);
                
            }

            SceneManager.push(Escene_MTTTTTT);
        }
    };

    // Escena principal del minijuego
    function Escene_MTTTTTT() {
        this.initialize.apply(this, arguments);
    }

    Escene_MTTTTTT.prototype = Object.create(Scene_Base.prototype);
    Escene_MTTTTTT.prototype.constructor = Escene_MTTTTTT;

    Escene_MTTTTTT.prototype.initialize = function() {
        Scene_Base.prototype.initialize.call(this);
    };

    Escene_MTTTTTT.prototype.terminate = function() {
        Scene_Base.prototype.terminate.call(this);
        Input.clear();
        TouchInput.clear();
    };

    Escene_MTTTTTT.prototype.create = function() {
    ...
    Escene_MTTTTTT.prototype.crearFondo = function() {
        this._fondo = new Sprite();
        const imagenFondo = ImageManager.loadBitmap('img/img_pluging/', 'tomaraguadelrio')
  ...
  Escene_MTTTTTT.prototype.createInfoButton = function() {
...
//metod del boton de enlaces
  Escene_MTTTTTT.prototype.createConsultaButton = function() {
  const buttonX = 225;  
  const buttonY = 100;  
  ...
  Escene_MTTTTTT.prototype.createFinalScoreBox = function() {
  ...
  Escene_MTTTTTT.prototype.createScoreBox = function() {
  ...
  scoreBox.bitmap.drawText("Puntaje actividad: " + oraguadelrio, 5, 5, 250, 40, 'left');


  this._scoreBox = scoreBox;
  this.addChild(scoreBox);
};
Escene_MTTTTTT.prototype.openConsultaWindow = function() {
  ...
  this.addChild(windowSprite);
      ...
      Escene_MTTTTTT.prototype.updateDragAndDrop = function() {
  ...
  Escene_MTTTTTT.prototype.resaltarCajasValidas = function(valor) {
  ...
  Escene_MTTTTTT.prototype.resetearResaltadoCajas = function() {
      for (const caja of this._cajas) {
          if (!caja._ocupada) {
              caja.bitmap.clear();
  ...
  Escene_MTTTTTT.prototype.update = function() {
    ...
    Escene_MTTTTTT.prototype.crearElementosArrastrables = function() {
        this._elementos = [];
        
        const valores = ['12', '8', '9', '13', '4,5', '15'];
        const posiciones = [
    ...
    Escene_MTTTTTT.prototype.crearCajasDestino = function() {
        this._cajas = [];
        ...
        const posicionesCajas = [
            ...
            // Columna 1
            {x: 260, y: 253 }, {x: 358, y: 312}, {x: 440, y: 253},
            
            // Columna 2
            {x: 360, y: 389}, {x: 440, y: 312}, {x: 360, y: 363},
    ...
    Escene_MTTTTTT.prototype.getBoxGroups = function() {
    return [
        ['9'],    // caja 1
        ['8'],    // caja 2
        ['13'],     // caja 3
      ...
      Escene_MTTTTTT.prototype.crearBotonesInferiores = function() {
        ...
        const y = TouchInput.y;
          if (TouchInput.isTriggered()) {
            if (x >= this.x && x <= this.x + this.width &&
                ...
                if (isValid) {
                _finalScore += oraguadelrio;
                const currentvida = $gameVariables.value(160);
                $gameVariables.setValue(160, currentvida + oraguadelrio);
                $gameSystem.establecerjuegoCompletadooraguadelrio(true);
                ...
                alert("✅ Prueba correcta. Se sumaron " + oraguadelrio + " puntos al resultado final.");
                } else {
                    //validar el contador 2
                $gameVariables.setValue(178, $gameVariables.value(178) - 1);
                oraguadelrio = Math.max(0, oraguadelrio - 1);
                // Verificar si se acabaron los intentos
                if (oraguadelrio <= 0 || $gameVariables.value(178) <= 0) {
                   $gameSystem.setlrmzJuegoBloqueadooraguadelrio(true);
                   SceneManager.push(Scene_LRMZ_BloqueoFinaloraguadelrio);
                   return;
                 }
                alert("❌ Respuestas incorrectas. Se restó 1 punto. Puntaje actual: " + oraguadelrio);
                if (oraguadelrio === 5) {
    ...
    Escene_MTTTTTT.prototype.validarRespuestas = function() {
      ...
      Escene_MTTTTTT.prototype.tryDrop = function(elementoArrastrable) {
  ...
  Escene_MTTTTTT.prototype.obtenerColorSegunTipo = function(valor) {
      // Colores diferentes para números vs porcentajes
      return valor.includes('%') ? '#f2f4f6ff' : '#fbf9f8ff'; //color al soltar
  };

  Escene_MTTTTTT.prototype.crearEfectoSoltado = function(x, y) {
  ...
  Escene_MTTTTTT.prototype.checkAllAnswers = function() {
 ...
 function Scene_LRMZ_BloqueoFinaloraguadelrio() {
  this.initialize.apply(this, arguments);
}

Scene_LRMZ_BloqueoFinaloraguadelrio.prototype = Object.create(Scene_Base.prototype);
Scene_LRMZ_BloqueoFinaloraguadelrio.prototype.constructor = Scene_LRMZ_BloqueoFinaloraguadelrio;

Scene_LRMZ_BloqueoFinaloraguadelrio.prototype.initialize = function() {
  Scene_Base.prototype.initialize.call(this);
};

Scene_LRMZ_BloqueoFinaloraguadelrio.prototype.create = function() {
  Scene_Base.prototype.create.call(this);

  // Imagen de fondo
  this._lrmzFondo = new Sprite();
  const bitmapFondo = ImageManager.loadBitmap('img/img_pluging/', 'tomaraguadelrioretro', false);
...
DataManager.makeSaveContents = function() {
  ...
  contents.oraguadelrio = oraguadelrio;
  contents._finalScore = _finalScore;
  contents._lrmzJuegoBloqueadooraguadelrio = $gameSystem.lrmzJuegoBloqueadooraguadelrio();
  return contents;
};
...
DataManager.extractSaveContents = function(contents) {
  ...
  oraguadelrio = contents.oraguadelrio || 0;
  _finalScore = contents._finalScore || 0;
  $gameSystem.setlrmzJuegoBloqueadooraguadelrio(contents._lrmzJuegoBloqueadooraguadelrio || false);
};

// Métodos de acceso para el bloqueo
Game_System.prototype.lrmzJuegoBloqueadooraguadelrio = function() {
  return this._lrmzJuegoBloqueadooraguadelrio || false;
};

Game_System.prototype.setlrmzJuegoBloqueadooraguadelrio = function(valor) {
  this._lrmzJuegoBloqueadooraguadelrio = valor;
};

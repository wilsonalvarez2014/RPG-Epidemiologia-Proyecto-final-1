/*:
 * @plugindesc [v2.0] QUIZ de Prevalencia
 * @author Luis
 * 
 ...
 *      $gameSystem.juegoCompletadoprevalencia()
 */
...
(function() {
    ...
    Object.defineProperty(window, 'prevalencia', {
        get: function() {
            return $gameVariables.value(181);
        },
        set: function(valor) {
            $gameVariables.setValue(181, valor);
        }
    });
    ...
    Game_System.prototype.juegoCompletadoprevalencia = function() {
        return this._juegoCompleto22prevalencia === true;
    };

    Game_System.prototype.establecerjuegoCompletadoprevalencia = function(estado15) {
        this._juegoCompleto22prevalencia = estado15;
    };

    Game_System.prototype.juegoBloqueadoprevalencia = function() {
        return this._juegoBloqueado22prevalencia === true;
    };

    Game_System.prototype.establecerBloqueoprevalencia = function(estado24) {
        this._juegoBloqueado22prevalencia = estado24;
    };
    ...
    Game_Interpreter.prototype.pluginCommand = function(comando, args) {
        ...
        if (comando === "Iniciarprevalencia") {
            if ($gameSystem.lrmzJuegoBloqueadoprevalencia()) {
                SceneManager.push(Scene_LRMZ_BloqueoFinalprevalencia);
                return;
            }
            
            if ($gameSystem.juegoCompletadoprevalencia()) {
                alert("Ya completaste este desafío con la máxima puntuación.");
                return;
            }
            // variable contador 2
            if ($gameVariables.value(182) === 0) {
                $gameVariables.setValue(182, 10) 
            }
            //establecer variable de juego
            if ($gameVariables.value(181) === 0) {
                $gameVariables.setValue(181, 10);
                
            }

            SceneManager.push(Escene_MTTTTTTTT);
        }
    };

    // Escena principal del minijuego
    function Escene_MTTTTTTTT() {
        this.initialize.apply(this, arguments);
    }

    Escene_MTTTTTTTT.prototype = Object.create(Scene_Base.prototype);
    Escene_MTTTTTTTT.prototype.constructor = Escene_MTTTTTTTT;

    Escene_MTTTTTTTT.prototype.initialize = function() {
        Scene_Base.prototype.initialize.call(this);
    };

    Escene_MTTTTTTTT.prototype.terminate = function() {
        Scene_Base.prototype.terminate.call(this);
        Input.clear();
        TouchInput.clear();
    };

    Escene_MTTTTTTTT.prototype.create = function() {
    ...
    Escene_MTTTTTTTT.prototype.crearFondo = function() {
        this._fondo = new Sprite();
        const imagenFondo = ImageManager.loadBitmap('img/img_pluging/', 'prevalencia')
  ...
  Escene_MTTTTTTTT.prototype.createInfoButton = function() {
...
//metod del boton de enlaces
  Escene_MTTTTTTTT.prototype.createConsultaButton = function() {
  const buttonX = 225;  
  const buttonY = 100;  
  ...
  Escene_MTTTTTTTT.prototype.createFinalScoreBox = function() {
  ...
  Escene_MTTTTTTTT.prototype.createScoreBox = function() {
  ...
  scoreBox.bitmap.drawText("Puntaje actividad: " + prevalencia, 5, 5, 250, 40, 'left');


  this._scoreBox = scoreBox;
  this.addChild(scoreBox);
};
Escene_MTTTTTTTT.prototype.openConsultaWindow = function() {
  ...
  this.addChild(windowSprite);
      ...
      Escene_MTTTTTTTT.prototype.updateDragAndDrop = function() {
  ...
  Escene_MTTTTTTTT.prototype.resaltarCajasValidas = function(valor) {
  ...
  Escene_MTTTTTTTT.prototype.resetearResaltadoCajas = function() {
      for (const caja of this._cajas) {
          if (!caja._ocupada) {
              caja.bitmap.clear();
  ...
  Escene_MTTTTTTTT.prototype.update = function() {
    ...
    Escene_MTTTTTTTT.prototype.crearElementosArrastrables = function() {
        this._elementos = [];
        
        const valores = ['12', '58,3', '7', '33,3', '4,8', '4'];
        const posiciones = [
            {x: 730, y: 290}, {x: 730, y: 324}, {x: 730, y: 356},
            {x: 730, y: 390}, {x: 730, y: 424}, {x: 730, y: 456},
    ...
    Escene_MTTTTTTTT.prototype.crearCajasDestino = function() {
        this._cajas = [];
        ...
        const posicionesCajas = [
            // Columna 1
            {x: 550, y: 193 }, {x: 550, y: 238}, {x: 550, y: 283},
            
            // Columna 2
            {x: 550, y: 328}, {x: 550, y: 476}, {x: 550, y: 521},
        ...
        for (let i = 0; i < 6; i++) {
            const caja = new Sprite(new Bitmap(100, 40));
    ...
    Escene_MTTTTTTTT.prototype.getBoxGroups = function() {
    return [
        ['4'],    // caja 1
        ['12'],    // caja 2
        ['7'],     // caja 3
      ...
      Escene_MTTTTTTTT.prototype.crearBotonesInferiores = function() {
        ...
        const y = TouchInput.y;
          if (TouchInput.isTriggered()) {
            if (x >= this.x && x <= this.x + this.width &&
                ...
                if (isValid) {
                _finalScore += prevalencia;
                const currentvida = $gameVariables.value(160);
                $gameVariables.setValue(160, currentvida + prevalencia);
                $gameSystem.establecerjuegoCompletadoprevalencia(true);
                ...
                alert("✅ Prueba correcta. Se sumaron " + prevalencia + " puntos al resultado final.");
                } else {
                    //validar el contador 2
                $gameVariables.setValue(182, $gameVariables.value(182) - 1);
                prevalencia = Math.max(0, prevalencia - 1);
                // Verificar si se acabaron los intentos
                if (prevalencia <= 0 || $gameVariables.value(182) <= 0) {
                   $gameSystem.setlrmzJuegoBloqueadoprevalencia(true);
                   SceneManager.push(Scene_LRMZ_BloqueoFinalprevalencia);
                   return;
                 }
                alert("❌ Respuestas incorrectas. Se restó 1 punto. Puntaje actual: " + prevalencia);
                if (prevalencia === 5) {
    ...
    Escene_MTTTTTTTT.prototype.validarRespuestas = function() {
      ...
      Escene_MTTTTTTTT.prototype.tryDrop = function(elementoArrastrable) {
  ...
  Escene_MTTTTTTTT.prototype.obtenerColorSegunTipo = function(valor) {
      // Colores diferentes para números vs porcentajes
      return valor.includes('%') ? '#f2f4f6ff' : '#fbf9f8ff'; //color al soltar
  };

  Escene_MTTTTTTTT.prototype.crearEfectoSoltado = function(x, y) {
  ...
  Escene_MTTTTTTTT.prototype.checkAllAnswers = function() {
 ...
 function Scene_LRMZ_BloqueoFinalprevalencia() {
  this.initialize.apply(this, arguments);
}

Scene_LRMZ_BloqueoFinalprevalencia.prototype = Object.create(Scene_Base.prototype);
Scene_LRMZ_BloqueoFinalprevalencia.prototype.constructor = Scene_LRMZ_BloqueoFinalprevalencia;

Scene_LRMZ_BloqueoFinalprevalencia.prototype.initialize = function() {
  Scene_Base.prototype.initialize.call(this);
};

Scene_LRMZ_BloqueoFinalprevalencia.prototype.create = function() {
  Scene_Base.prototype.create.call(this);

  // Imagen de fondo
  this._lrmzFondo = new Sprite();
  const bitmapFondo = ImageManager.loadBitmap('img/img_pluging/', 'Prevalencia retro', false);
...
DataManager.makeSaveContents = function() {
  ...
  contents.prevalencia = prevalencia;
  contents._finalScore = _finalScore;
  contents._lrmzJuegoBloqueadoprevalencia = $gameSystem.lrmzJuegoBloqueadoprevalencia();
  return contents;
};
...
DataManager.extractSaveContents = function(contents) {
  ...
  prevalencia = contents.prevalencia || 0;
  _finalScore = contents._finalScore || 0;
  $gameSystem.setlrmzJuegoBloqueadoprevalencia(contents._lrmzJuegoBloqueadoprevalencia || false);
};

// Métodos de acceso para el bloqueo
Game_System.prototype.lrmzJuegoBloqueadoprevalencia = function() {
  return this._lrmzJuegoBloqueadoprevalencia || false;
};

Game_System.prototype.setlrmzJuegoBloqueadoprevalencia = function(valor) {
  this._lrmzJuegoBloqueadoprevalencia = valor;
};

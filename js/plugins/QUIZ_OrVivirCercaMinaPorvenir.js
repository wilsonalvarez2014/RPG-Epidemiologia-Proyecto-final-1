/*:
 * @plugindesc [v2.0] QUIZ de vivir cerca a la mina el porvenir 
 * @author Luis
 *
 * 
 ...
 *      $gameSystem.juegoCompletadoorcercaminaporvenir()
 */
...
(function() {
    ...
    Object.defineProperty(window, 'orcercaminaporvenir', {
        get: function() {
            return $gameVariables.value(179);
        },
        set: function(valor) {
            $gameVariables.setValue(179, valor);
        }
    });
    ...
    Game_System.prototype.juegoCompletadoorcercaminaporvenir = function() {
        return this._juegoCompleto22orcercaminaporvenir === true;
    };

    Game_System.prototype.establecerjuegoCompletadoorcercaminaporvenir = function(estado15) {
        this._juegoCompleto22orcercaminaporvenir = estado15;
    };

    Game_System.prototype.juegoBloqueadoorcercaminaporvenir = function() {
        return this._juegoBloqueado22orcercaminaporvenir === true;
    };

    Game_System.prototype.establecerBloqueoorcercaminaporvenir = function(estado24) {
        this._juegoBloqueado22orcercaminaporvenir = estado24;
    };
    ...
    Game_Interpreter.prototype.pluginCommand = function(comando, args) {
        ...
        if (comando === "Iniciarorcercamina") {
            if ($gameSystem.lrmzJuegoBloqueadoorcercaminaporvenir()) {
                SceneManager.push(Scene_LRMZ_BloqueoFinalorcercaminaporvenir);
                return;
            }
            
            if ($gameSystem.juegoCompletadoorcercaminaporvenir()) {
                alert("Ya completaste este desafío con la máxima puntuación.");
                return;
            }
            // variable contador 2
            if ($gameVariables.value(180) === 0) {
                $gameVariables.setValue(180, 10) 
            }
            //establecer variable de juego
            if ($gameVariables.value(179) === 0) {
                $gameVariables.setValue(179, 10);
                
            }

            SceneManager.push(Escene_MTTTTTTT);
        }
    };

    // Escena principal del minijuego
    function Escene_MTTTTTTT() {
        this.initialize.apply(this, arguments);
    }

    Escene_MTTTTTTT.prototype = Object.create(Scene_Base.prototype);
    Escene_MTTTTTTT.prototype.constructor = Escene_MTTTTTTT;

    Escene_MTTTTTTT.prototype.initialize = function() {
        Scene_Base.prototype.initialize.call(this);
    };

    Escene_MTTTTTTT.prototype.terminate = function() {
        Scene_Base.prototype.terminate.call(this);
        Input.clear();
        TouchInput.clear();
    };

    Escene_MTTTTTTT.prototype.create = function() {
    ...
    Escene_MTTTTTTT.prototype.crearFondo = function() {
        this._fondo = new Sprite();
        const imagenFondo = ImageManager.loadBitmap('img/img_pluging/', 'cercaminaporvenir')
  ...
  Escene_MTTTTTTT.prototype.createInfoButton = function() {
...
//metod del boton de enlaces
  Escene_MTTTTTTT.prototype.createConsultaButton = function() {
  const buttonX = 225;  
  const buttonY = 100;  
  ...
  Escene_MTTTTTTT.prototype.createFinalScoreBox = function() {
  ...
  Escene_MTTTTTTT.prototype.createScoreBox = function() {
  ...
  scoreBox.bitmap.drawText("Puntaje actividad: " + orcercaminaporvenir, 5, 5, 250, 40, 'left');


  this._scoreBox = scoreBox;
  this.addChild(scoreBox);
};
Escene_MTTTTTTT.prototype.openConsultaWindow = function() {
  ...
  this.addChild(windowSprite);
      ...
      Escene_MTTTTTTT.prototype.updateDragAndDrop = function() {
  ...
  Escene_MTTTTTTT.prototype.resaltarCajasValidas = function(valor) {
  ...
  Escene_MTTTTTTT.prototype.resetearResaltadoCajas = function() {
      for (const caja of this._cajas) {
          if (!caja._ocupada) {
              caja.bitmap.clear();
  ...
  Escene_MTTTTTTT.prototype.update = function() {
    ...
    Escene_MTTTTTTT.prototype.crearElementosArrastrables = function() {
        this._elementos = [];
        
        const valores = ['13', '9', '11', '8', '4,8', '14'];
        const posiciones = [
            {x: 680, y: 243}, {x: 680, y: 277}, {x: 680, y: 309},
            {x: 680, y: 343}, {x: 680, y: 377}, {x: 680, y: 412},
    ...
    Escene_MTTTTTTT.prototype.crearCajasDestino = function() {
        this._cajas = [];
        ...
        const posicionesCajas = [
            // Columna 1
            {x: 260, y: 243 }, {x: 358, y: 300}, {x: 439, y: 243},
            
            // Columna 2
            {x: 264, y: 335}, {x: 439, y: 300}, {x: 360, y: 358},
        ...
        for (let i = 0; i < 6; i++) {
            const caja = new Sprite(new Bitmap(71, 20));
    ...
    Escene_MTTTTTTT.prototype.getBoxGroups = function() {
    return [
        ['8'],    // caja 1
        ['9'],    // caja 2
        ['11'],     // caja 3
      ...
      Escene_MTTTTTTT.prototype.crearBotonesInferiores = function() {
        ...
        const y = TouchInput.y;
          if (TouchInput.isTriggered()) {
            if (x >= this.x && x <= this.x + this.width &&
                ...
                if (isValid) {
                _finalScore += orcercaminaporvenir;
                const currentvida = $gameVariables.value(160);
                $gameVariables.setValue(160, currentvida + orcercaminaporvenir);
                $gameSystem.establecerjuegoCompletadoorcercaminaporvenir(true);
                ...
                alert("✅ Prueba correcta. Se sumaron " + orcercaminaporvenir + " puntos al resultado final.");
                } else {
                    //validar el contador 2
                $gameVariables.setValue(180, $gameVariables.value(180) - 1);
                orcercaminaporvenir = Math.max(0, orcercaminaporvenir - 1);
                // Verificar si se acabaron los intentos
                if (orcercaminaporvenir <= 0 || $gameVariables.value(180) <= 0) {
                   $gameSystem.setlrmzJuegoBloqueadoorcercaminaporvenir(true);
                   SceneManager.push(Scene_LRMZ_BloqueoFinalorcercaminaporvenir);
                   return;
                 }
                alert("❌ Respuestas incorrectas. Se restó 1 punto. Puntaje actual: " + orcercaminaporvenir);
                if (orcercaminaporvenir === 5) {
    ...
    Escene_MTTTTTTT.prototype.validarRespuestas = function() {
      ...
      Escene_MTTTTTTT.prototype.tryDrop = function(elementoArrastrable) {
  ...
  Escene_MTTTTTTT.prototype.obtenerColorSegunTipo = function(valor) {
      // Colores diferentes para números vs porcentajes
      return valor.includes('%') ? '#f2f4f6ff' : '#fbf9f8ff'; //color al soltar
  };

  Escene_MTTTTTTT.prototype.crearEfectoSoltado = function(x, y) {
  ...
  Escene_MTTTTTTT.prototype.checkAllAnswers = function() {
 ...
 function Scene_LRMZ_BloqueoFinalorcercaminaporvenir() {
  this.initialize.apply(this, arguments);
}

Scene_LRMZ_BloqueoFinalorcercaminaporvenir.prototype = Object.create(Scene_Base.prototype);
Scene_LRMZ_BloqueoFinalorcercaminaporvenir.prototype.constructor = Scene_LRMZ_BloqueoFinalorcercaminaporvenir;

Scene_LRMZ_BloqueoFinalorcercaminaporvenir.prototype.initialize = function() {
  Scene_Base.prototype.initialize.call(this);
};

Scene_LRMZ_BloqueoFinalorcercaminaporvenir.prototype.create = function() {
  Scene_Base.prototype.create.call(this);

  // Imagen de fondo
  this._lrmzFondo = new Sprite();
  const bitmapFondo = ImageManager.loadBitmap('img/img_pluging/', 'cercaminaporvenirretro', false);
...
DataManager.makeSaveContents = function() {
  ...
  contents.orcercaminaporvenir = orcercaminaporvenir;
  contents._finalScore = _finalScore;
  contents._lrmzJuegoBloqueadoorcercaminaporvenir = $gameSystem.lrmzJuegoBloqueadoorcercaminaporvenir();
  return contents;
};
...
DataManager.extractSaveContents = function(contents) {
  ...
  orcercaminaporvenir = contents.orcercaminaporvenir || 0;
  _finalScore = contents._finalScore || 0;
  $gameSystem.setlrmzJuegoBloqueadoorcercaminaporvenir(contents._lrmzJuegoBloqueadoorcercaminaporvenir || false);
};

// Métodos de acceso para el bloqueo
Game_System.prototype.lrmzJuegoBloqueadoorcercaminaporvenir = function() {
  return this._lrmzJuegoBloqueadoorcercaminaporvenir || false;
};

Game_System.prototype.setlrmzJuegoBloqueadoorcercaminaporvenir = function(valor) {
  this._lrmzJuegoBloqueadoorcercaminaporvenir = valor;
};

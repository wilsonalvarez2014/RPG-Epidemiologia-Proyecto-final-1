/*:
 * @plugindesc [v2.0] QUIZ de factor de exposicion plaguicidas
 * @author Luis
 * 
 * 
 ...
 *     "Iniciarorplaguicidas"
 * 
 * Metodo validacion quiz terminado
 *      $gameSystem.juegoCompletadoorplaguicidas()
 * 
 */
...
(function() {
    ...
    Object.defineProperty(window, 'orplaguicidas', {
        get: function() {
            return $gameVariables.value(175);
        },
        set: function(valor) {
            $gameVariables.setValue(175, valor);
        }
    });
    ...
    Game_System.prototype.juegoCompletadoorplaguicidas = function() {
        return this._juegoCompleto22orplaguicidas === true;
    };

    Game_System.prototype.establecerjuegoCompletadoorplaguicidas = function(estado13) {
        this._juegoCompleto22orplaguicidas = estado13;
    };

    Game_System.prototype.juegoBloqueadoorplaguicidas = function() {
        return this._juegoBloqueado22orplaguicidas === true;
    };

    Game_System.prototype.establecerBloqueoorplaguicidas = function(estado24) {
        this._juegoBloqueado22orplaguicidas = estado24;
    };
    ...
    Game_Interpreter.prototype.pluginCommand = function(comando, args) {
        ...
        if (comando === "Iniciarorplaguicidas") {
            if ($gameSystem.lrmzJuegoBloqueadoorplaguicidas()) {
                SceneManager.push(Scene_LRMZ_BloqueoFinalorplaguicidas);
                return;
            }
            
            if ($gameSystem.juegoCompletadoorplaguicidas()) {
                alert("Ya completaste este desafío con la máxima puntuación.");
                return;
            }
            // variable contador 2
            if ($gameVariables.value(176) === 0) {
                $gameVariables.setValue(176, 10) 
            }
            //establecer variable de juego
            if ($gameVariables.value(175) === 0) {
                $gameVariables.setValue(175, 10);
                
            }

            SceneManager.push(Escene_MTTTTT);
        }
    };

    // Escena principal del minijuego
    function Escene_MTTTTT() {
        this.initialize.apply(this, arguments);
    }

    Escene_MTTTTT.prototype = Object.create(Scene_Base.prototype);
    Escene_MTTTTT.prototype.constructor = Escene_MTTTTT;

    Escene_MTTTTT.prototype.initialize = function() {
        Scene_Base.prototype.initialize.call(this);
    };

    Escene_MTTTTT.prototype.terminate = function() {
        Scene_Base.prototype.terminate.call(this);
        Input.clear();
        TouchInput.clear();
    };

    Escene_MTTTTT.prototype.create = function() {
    ...
    Escene_MTTTTT.prototype.crearFondo = function() {
        this._fondo = new Sprite();
        const imagenFondo = ImageManager.loadBitmap('img/img_pluging/', 'or persona expuesta a plagicidas')
  ...
  Escene_MTTTTT.prototype.createInfoButton = function() {
...
//metod del boton de enlaces
  Escene_MTTTTT.prototype.createConsultaButton = function() {
  const buttonX = 225;  
  const buttonY = 100;  
  ...
  Escene_MTTTTT.prototype.createFinalScoreBox = function() {
  ...
  Escene_MTTTTT.prototype.createScoreBox = function() {
  ...
  scoreBox.bitmap.drawText("Puntaje actividad: " + orplaguicidas, 5, 5, 250, 40, 'left');


  this._scoreBox = scoreBox;
  this.addChild(scoreBox);
};
Escene_MTTTTT.prototype.openConsultaWindow = function() {
  ...
  this.addChild(windowSprite);
      ...
      Escene_MTTTTT.prototype.updateDragAndDrop = function() {
  ...
  Escene_MTTTTT.prototype.resaltarCajasValidas = function(valor) {
  ...
  Escene_MTTTTT.prototype.resetearResaltadoCajas = function() {
      for (const caja of this._cajas) {
          if (!caja._ocupada) {
              caja.bitmap.clear();
  ...
  Escene_MTTTTT.prototype.update = function() {
    ...
    Escene_MTTTTT.prototype.crearElementosArrastrables = function() {
        this._elementos = [];
        
        const valores = ['12', '7', '11', '8', '6', '0,4'];
        const posiciones = [
            {x: 680, y: 245}, {x: 680, y: 280}, {x: 680, y: 312},
            {x: 680, y: 345}, {x: 680, y: 380}, {x: 680, y: 415},
    ...
    Escene_MTTTTT.prototype.crearCajasDestino = function() {
        this._cajas = [];
        ...
        const posicionesCajas = [
            // Columna 1
            {x: 270, y: 243 }, {x: 360, y: 243}, {x: 449, y: 243},
            
            // Columna 2
            {x: 270, y: 280}, {x: 360, y: 315}, {x: 360, y: 342},
    ...
    Escene_MTTTTT.prototype.getBoxGroups = function() {
    return [
        ['6'],    // caja 1
        ['8'],    // caja 2
        ['11'],     // caja 3
      ...
      Escene_MTTTTT.prototype.crearBotonesInferiores = function() {
        ...
        const y = TouchInput.y;
          if (TouchInput.isTriggered()) {
            if (x >= this.x && x <= this.x + this.width &&
                ...
                if (isValid) {
                _finalScore += orplaguicidas;
                const currentvida = $gameVariables.value(160);
                $gameVariables.setValue(160, currentvida + orplaguicidas);
                $gameSystem.establecerjuegoCompletadoorplaguicidas(true);
                ...
                alert("✅ Prueba correcta. Se sumaron " + orplaguicidas + " puntos al resultado final.");
                } else {
                    //validar el contador 2
                $gameVariables.setValue(176, $gameVariables.value(176) - 1);
                orplaguicidas = Math.max(0, orplaguicidas - 1);
                // Verificar si se acabaron los intentos
                if (orplaguicidas <= 0 || $gameVariables.value(176) <= 0) {
                   $gameSystem.setlrmzJuegoBloqueadoorplaguicidas(true);
                   SceneManager.push(Scene_LRMZ_BloqueoFinalorplaguicidas);
                   return;
                 }
                alert("❌ Respuestas incorrectas. Se restó 1 punto. Puntaje actual: " + orplaguicidas);
                if (orplaguicidas === 5) {
    ...
    Escene_MTTTTT.prototype.validarRespuestas = function() {
      ...
      Escene_MTTTTT.prototype.tryDrop = function(elementoArrastrable) {
  ...
  Escene_MTTTTT.prototype.obtenerColorSegunTipo = function(valor) {
      // Colores diferentes para números vs porcentajes
      return valor.includes('%') ? '#f2f4f6ff' : '#fbf9f8ff'; //color al soltar
  };

  Escene_MTTTTT.prototype.crearEfectoSoltado = function(x, y) {
  ...
  Escene_MTTTTT.prototype.checkAllAnswers = function() {
 ...
 function Scene_LRMZ_BloqueoFinalorplaguicidas() {
  this.initialize.apply(this, arguments);
}

Scene_LRMZ_BloqueoFinalorplaguicidas.prototype = Object.create(Scene_Base.prototype);
Scene_LRMZ_BloqueoFinalorplaguicidas.prototype.constructor = Scene_LRMZ_BloqueoFinalorplaguicidas;

Scene_LRMZ_BloqueoFinalorplaguicidas.prototype.initialize = function() {
  Scene_Base.prototype.initialize.call(this);
};

Scene_LRMZ_BloqueoFinalorplaguicidas.prototype.create = function() {
  Scene_Base.prototype.create.call(this);

  // Imagen de fondo
  this._lrmzFondo = new Sprite();
  const bitmapFondo = ImageManager.loadBitmap('img/img_pluging/', 'or persona expuesta a plaguicidas retro', false);
...
DataManager.makeSaveContents = function() {
  ...
  contents.orplaguicidas = orplaguicidas;
  contents._finalScore = _finalScore;
  contents._lrmzJuegoBloqueadoorplaguicidas = $gameSystem.lrmzJuegoBloqueadoorplaguicidas();
  return contents;
};
...
DataManager.extractSaveContents = function(contents) {
  ...
  orplaguicidas = contents.orplaguicidas || 0;
  _finalScore = contents._finalScore || 0;
  $gameSystem.setlrmzJuegoBloqueadoorplaguicidas(contents._lrmzJuegoBloqueadoorplaguicidas || false);
};

// Métodos de acceso para el bloqueo
Game_System.prototype.lrmzJuegoBloqueadoorplaguicidas = function() {
  return this._lrmzJuegoBloqueadoorplaguicidas || false;
};

Game_System.prototype.setlrmzJuegoBloqueadoorplaguicidas = function(valor) {
  this._lrmzJuegoBloqueadoorplaguicidas = valor;
};
